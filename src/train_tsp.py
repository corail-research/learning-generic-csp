...
from models.decision_tsp.config import TSPConfig
from src.models.common.pytorch_utilities import PairBatchSampler
...
if __name__ == "__main__":
    ...
    hidden_size = {key: num_hidden_channels for key in first_batch.x_dict.leys()}
    out_channels = {key: num_hidden_channels for key in first_batch.x_dict.keys()}
    
    model = GNNTSP(metadata, input_size, out_channels, hidden_size, num_passes=num_lstm_passes, device=device, aggr=gnn_aggregation)
    model = model.cuda()
    ...
    train_model(model,train_loader,test_loader,optimizer,lr_scheduler,criterion,num_epochs,samples_per_epoch=samples_per_epoch,clip_value=experiment_config.clip_gradient_norm,model_save_path=model_save_path,wandb_run_name=wandb.run.name)
