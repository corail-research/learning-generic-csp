...
class GenericGNNTSP(AdaptedNeuroSAT):
    def __init__(self,
        ...
        self.projection_division = {}
        self.num_mlp_layers = num_mlp_layers
        self.vote = MLP(hidden_size["x"], num_mlp_layers, 1, hidden_size["x"], device=device)
        lstm_hidden_sizes = {node_type: hidden_size[node_type] for node_type in metadata[0]}
        self.lstm_conv_layers = DTSPLSTMConv(lstm_hidden_sizes, lstm_hidden_sizes, metadata=metadata, device=device, layernorm_lstm_cell=layernorm_lstm_cell, num_mlp_layers=num_mlp_layers)
        for node_type in metadata[0]:
