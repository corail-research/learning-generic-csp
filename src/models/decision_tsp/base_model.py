...
class GNNTSP(AdaptedNeuroSAT):
    def __init__(self, metadata, in_channels:Dict[str, int], out_channels:Dict[str, int], hidden_size:Dict[str, int]=128, num_passes:int=20, device="cpu", flip_inputs:bool=False):
        ...
        self.lstm_conv_layers = DTSPLSTMConv(lstm_hidden_sizes, lstm_hidden_sizes, metadata=metadata, device=device, flip_inputs=flip_inputs)
        for node_type in metadata[0]:
            if node_type == "city":
                self.projection_layers[node_type] = torch.nn.Linear(in_channels[node_type], hidden_size[node_type])
            else:
                self.projection_layers[node_type] = MLP(hidden_size["arc"], 2, 1, hidden_size["arc"], device=device)
